...
export default function ChatPanel() {
  ...
  const [fileProgress, setFileProgress] = useState<{ fileName: string; stage: 'reading' | 'parsing' } | null>(null);
  ...
  useEffect(() => {
    if (!isStreaming && activeWorkspace && activeSessionId && items.length > 0) {
      window.api.saveSessionMessages(activeWorkspace.id, activeSessionId, toStoredItems());
    }
  }, [isStreaming]);
  ...
  const send = async () => {
    ...
    window.api.sendMessage(allMessages, activeSessionId ?? undefined).catch(() => finalizeAssistantMessage());
  };
    ...
    return (
      <div className="flex-1 flex items-center justify-center text-gray-400 bg-gray-50">
        <div className="text-center space-y-3">
          ...
          <p className="text-xs text-gray-300">Workspace là thư mục chứa tài liệu giảng dạy của bạn</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex-1 flex flex-col bg-gray-50 min-w-0">
      ...
      <div className="bg-white border-b px-4 py-2.5 flex items-center gap-3 flex-shrink-0">
        <div className="min-w-0">
          ...
          <p className="text-xs text-gray-400 font-mono truncate">{activeWorkspace.path}</p>
        </div>
      </div>
        ...
        {items.length === 0 && !isStreaming && (
          <div className="flex flex-col items-center justify-center h-full text-gray-400 space-y-4">
            ...
            <div className="text-center">
              <p className="text-sm font-medium text-gray-600">Tôi có thể giúp gì cho bạn hôm nay?</p>
              <p className="text-xs text-gray-400 mt-1">Tôi sẽ tự tìm tài liệu trong workspace của bạn</p>
            </div>
        ...
        {isStreaming && (
          <>
            ...
            {fileProgress && (
              <div className="flex items-center gap-2 text-xs text-gray-400 py-1 mb-1">
                <span className="inline-flex gap-0.5">
                  <span className="w-1 h-1 bg-blue-300 rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                  <span className="w-1 h-1 bg-blue-300 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                  <span className="w-1 h-1 bg-blue-300 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                </span>
                <span>
                  {fileProgress.stage === 'reading' ? 'Đang đọc' : 'Đang phân tích'}{' '}
                  <span className="font-mono text-gray-500">{fileProgress.fileName}</span>
                </span>
              </div>
            )}
