...
import type { ToolCallItem } from '../stores/chatStore';
...
export function ThinkingBox({ thinking, isOpen }: ThinkingBoxProps) {
        ...
        <div className="mt-1 ml-4 bg-gray-50 border-l-2 border-gray-200 overflow-hidden text-xs">
          {Object.keys(item.args).length > 0 && (
            <div className="px-3 py-2 border-b border-gray-100">
              <p className="text-gray-400 font-medium mb-1 uppercase tracking-wide" style={{ fontSize: '10px' }}>Đầu vào</p>
          ...
          {item.result && (
            <div className="px-3 py-2">
              <p className="text-gray-400 font-medium mb-1 uppercase tracking-wide" style={{ fontSize: '10px' }}>Kết quả</p>
              <pre className="text-gray-600 whitespace-pre-wrap font-mono leading-relaxed">{item.result}</pre>
            </div>
          )}
        </div>
      )}
...
interface MessageBubbleProps {
  message: ChatMessage & { thinking?: string };
}
...
export function StreamingBubble({ content }: StreamingBubbleProps) {
  ...
  const hasThinking = parsed.thinking || parsed.thinkingOpen;
  const hasText = parsed.text.length > 0;

  return (
    <div className="mb-4">
      {hasThinking && (
        <ThinkingBox thinking={parsed.thinking} isOpen={parsed.thinkingOpen} />
      )}
      {hasText ? (
        <div className="prose prose-sm max-w-none text-gray-800">
          <ReactMarkdown remarkPlugins={[remarkGfm, remarkMath]} rehypePlugins={[rehypeKatex]}>{parsed.text}</ReactMarkdown>
        </div>
      ) : !hasThinking ? (
